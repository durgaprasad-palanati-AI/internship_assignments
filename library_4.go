...
// book interface
type book interface {
	getbook_details() books
}
type books struct {
	book_id uint //variable to store book's id
}

func (bb books) getbook_details() books {
	return bb
}
func getdetails_book(b book) {

	fmt.Println(b.getbook_details())
}
...
func main() {
	...
	new_bookentry := Library{
		add_newbook: func(b books) {
			...
			fmt.Scanf("%d\n", &b.book_id) //enter book id & store it in book_id
	...
	for true {
		...
		fmt.Println("6.details of books")
		fmt.Println("7.EXIT")
		fmt.Scanf("%d\n", &i)
		switch i {
		case 1:
